 ...
 * @LastEditTime : 2020-02-10 13:23:58
 -->
<template>
  <el-form @submit.native.prevent="saveProject" ref="form" :model="project" label-width="120px">
    ...
    <el-form-item label="项目环境变量">
      <span style="margin-left:20px; font-size:12px; color:#919191">注意： <el-badge is-dot class="item"></el-badge>表示必填项</span>
      <div v-for="(item, index) in env_dict" :key="index" style="margin:10px 0">
        <span style="padding:0 15px">键</span>
        <el-input v-model="item.key" :inline="true" :value="item.key" style="width:40%" placeholder="key"></el-input>
        <span style="padding:0 15px; padding-left:40px">值
          <el-badge is-dot class="item" v-show="item.required"></el-badge>
        </span>
        <el-input v-model="item.value" :inline="true" :value="item.value" style="width:40%" placeholder="value"></el-input>
        <el-button v-show="!item.required" style="margin-left:20px" type="danger" icon="el-icon-minus" @click="minus_env(index,item.required)" circle></el-button>
      </div>
